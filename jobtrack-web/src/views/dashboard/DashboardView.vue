...
const funnelElement = ref<HTMLElement | null>(null); const trendElement = ref<HTMLElement | null>(null); const sourceElement = ref<HTMLElement | null>(null); const industryElement = ref<HTMLElement | null>(null); const charts = new Set<echarts.ECharts>()

const summaryCards = computed(() => { const item = summary.value; return [{ label: '投递总数', value: item?.totalApplications ?? 0 }, { label: '进行中', value: item?.inProgressApplications ?? 0 }, { label: '待处理', value: item?.pendingTasks ?? 0 }, { label: '未来面试', value: item?.upcomingInterviews ?? 0 }, { label: 'Offer', value: item?.offerApplications ?? 0 }, { label: '接受 Offer', value: item?.acceptedOffers ?? 0 }, { label: '面试转化', value: `${(item?.interviewConversionRate ?? 0).toFixed(2)}%` }, { label: 'Offer 转化', value: `${(item?.offerConversionRate ?? 0).toFixed(2)}%` }] })
...
function renderCharts() { renderChart(funnelElement.value, { tooltip: {}, xAxis: { type: 'value' }, yAxis: { type: 'category', data: funnel.value.map((item) => item.label) }, series: [{ type: 'bar', data: funnel.value.map((item) => item.count), itemStyle: { color: '#409eff' } }] }); renderChart(trendElement.value, { tooltip: { trigger: 'axis' }, xAxis: { type: 'category', data: trends.value.map((item) => item.bucket) }, yAxis: { type: 'value' }, series: [{ type: 'line', smooth: true, data: trends.value.map((item) => item.count), itemStyle: { color: '#67c23a' }, areaStyle: { opacity: 0.12 } }] }); renderChart(sourceElement.value, pieOption(sources.value)); renderChart(industryElement.value, pieOption(industries.value)) }
...
function renderChart(element: HTMLElement | null, option: ChartOption) { if (!element) return; const chart = echarts.getInstanceByDom(element) || echarts.init(element); charts.add(chart); chart.setOption(option, true) }
